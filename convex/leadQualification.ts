...
export const LEAD_EVIDENCE_MAX_AGE_DAYS = 90;
...
export function evaluateLeadGate(input: LeadGateInput) {
  ...
  if (input.observedAt < input.now - LEAD_EVIDENCE_MAX_AGE_DAYS * DAY) {
    blockers.push(`Market evidence is older than ${LEAD_EVIDENCE_MAX_AGE_DAYS} days.`);
  }
